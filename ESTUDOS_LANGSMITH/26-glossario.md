...
Um glossário serve para **usar o termo certo na conversa certa** — e o risco, em projetos com mais de uma ferramenta, é misturar vocabulários sem perceber que a semântica mudou junto.

**Na plataforma de atendimento (Langfuse Cloud).** Aquele projeto tem o próprio `GLOSSARY.md` e um vocabulário de domínio que não está neste curso: N1–N5 (níveis de autonomia), *guided booking*, *reuse_answer*, *freeform_fallback*, janela de veto, `mechanism` (`ungoverned_n5` / `governed_autonomy`), Q&A × clínico *parent-child*, `Hit@k`, `ACCEPTABLE`/`UNACCEPTABLE`/`INCONCLUSIVE`, `critical_checks`, `expected_facts`, label `ativo`. Monte a tabela de tradução entre os três vocabulários — este glossário, o do Langfuse e o do domínio — porque é nela que os erros se escondem: *run* ↔ *observation*, mas uma `AIGeneration` da aplicação **não** é uma *generation* do Langfuse quando o provider foi determinístico; *feedback* ↔ *score*, mas `n5.pending_outcome` é um score categórico de **desfecho operacional**, não uma nota de qualidade; *thread* ↔ *session*, mas a *session* é a conversa inteira e o *trace* é um turno — a unidade de "continuidade" é a session, a de "utilidade" é o turno; *fixture* aqui é o parser determinístico, lá é conhecimento + ofertas + estado de conversa com `fixture_version`; *slice* aqui é `critico`/`portugues`, lá é caminho N5 — e o plano já avisou que talvez devesse ser faixa de `Hit@k`. E uma entrada que só existe lá e merece entrar em qualquer glossário de chat: **correlação tardia** — a mensagem é criada numa requisição posterior à geração, e o vínculo é por ID explícito, não por contexto.

**Em projetos comuns do ecossistema.** Três confusões recorrentes que um glossário compartilhado evita: *trace* (árvore de uma execução) × *thread/session* (várias execuções de um caso); *offline* (fora do tráfego; pode chamar modelo e rede) × *local* (sem rede); *avaliação* (medida) × *gate* (política sobre a medida). E o cuidado permanente: "campos com nomes parecidos em produtos diferentes podem ter semânticas distintas" — `latency` no LangSmith inclui filhos; em um score seu, inclua ou exclua explicitamente.

**O fator humano — onde a IA faz e onde você decide.** A IA gera glossários completos e corretos — e usa os termos com fluência mesmo quando os mistura, porque para ela "run", "span", "observation" e "generation" são vizinhos no espaço semântico, não contratos com campos diferentes. Onde você é o diferencial: **traduzir entre pessoas**. Na reunião com quem cuida do atendimento, "o turno caiu em fallback livre porque o atalho clínico ficou abaixo de 0,40" precisa virar "o sistema não achou uma resposta confiável e respondeu de forma genérica"; com quem opera o Langfuse, o inverso. Foque em: (1) manter a tabela de tradução dos três vocabulários — é pequena e é sua; (2) ao ler um número, perguntar em qual vocabulário ele foi medido (uma "taxa de erro por observation" e uma "por turno" são as quatro taxas do capítulo 22 outra vez); (3) corrigir termos em documentos gerados — um plano que diz "o Langfuse retoma a conversa" ou "a *generation* prova que houve LLM" está errado de um jeito que passa despercebido a quem não leu o capítulo 06 e o 04. Apontar um exemplo concreto para cada termo, como a introdução pede, é o teste que separa saber a palavra de saber a coisa.
