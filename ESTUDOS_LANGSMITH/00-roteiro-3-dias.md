...
Este roteiro foi desenhado para uma entrevista e para o DCRA, mas o esqueleto — **enxergar a execução, medir se a mudança melhorou, defender a decisão** — é o mesmo de qualquer projeto que use LangSmith, Langfuse, LangGraph ou LangChain. O que muda é o sistema sob a lente e a ferramenta que segura a lente.

**Na plataforma de atendimento (Langfuse Cloud).** O `LANGFUSE/plano_implementacao.md` daquele repositório já tem a sua própria versão destes três dias: a Fase 0 é o Dia 1 (instrumentar `rag.retrieve`, as chamadas de provider e a decisão em `maybe_open_autonomous_window()`; ler 20–30 conversas N5); o contrato de avaliação do `detalhamento_execucao.md` §6 é o Dia 2 (dataset `n5-baseline-v1`, rubrica 0/1/2, juiz calibrado em 12 exemplos); e a gestão de prompts do §7 mais o relatório por caminho são o Dia 3 (comparar, promover com label `ativo`, reverter). A diferença de ordem importa: lá, o Dia 1 termina com **três modos de falha com denominador**, não com um trace bonito. Se você chegar ao fim da Fase 0 sem conseguir dizer "a maioria das respostas ruins vem da recuperação/do contexto/da geração", o Dia 2 não tem o que medir.

**Em projetos comuns do ecossistema.** A rota curta de 90 minutos é reutilizável quase literalmente: um trace manual com `@traceable` ou `@observe`, uma execução do fluxo real com dependências falsas, um avaliador de código que pega um erro plantado, e a conta de custo. Em qualquer projeto novo, faça isso **antes** de configurar dashboards, juízes online ou pipelines: quatro evidências pequenas valem mais que um painel vazio.

**O fator humano — onde a IA faz e onde você decide.** Um assistente de IA produz um roteiro como este em minutos, e produziria outro igualmente convincente com a ordem trocada. O que ele não sabe: quanto tempo você **realmente** tem, o que a vaga ou o projeto vai cobrar de verdade, e qual etapa você ainda não consegue explicar sem abrir o arquivo. Esse é o seu denominador — só você conhece. Foque em três coisas que nenhum roteiro gerado faz por você: (1) escolher o que cortar quando o tempo acabar (a rota curta é uma proposta; a sua pode ser outra); (2) executar de fato os comandos e escrever "o que observei" com números que você viu; (3) treinar a explicação em voz alta, porque a fluência é o único artefato que a IA não pode entregar no seu lugar. Na plataforma de atendimento, o equivalente é a sessão LF-6: a IA pode instrumentar os três pontos, mas ler 20 conversas e nomear os modos de falha é trabalho de quem conhece o atendimento.
