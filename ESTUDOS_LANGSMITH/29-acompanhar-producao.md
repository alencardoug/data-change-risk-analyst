...
"Alguém usa isto?" tem três respostas com três denominadores — e antes de qualquer uma delas, um teto de gasto. É a pergunta mais simples de operação e a que mais projetos com LLM não conseguem responder.

**Na plataforma de atendimento (Langfuse Cloud).** As três camadas existem lá, com fontes próprias. **Tráfego:** o backend FastAPI tem `/health` e `/ready`, logs estruturados com `request_id` e, em produção, o proxy/hospedagem que estiver na frente conta requisições — com as mesmas ressalvas (polls do cliente e do operador são requisições, não pessoas; `/customer` aberto sem mensagem é visita, não conversa). **Uso real:** `customer_service.ai_generations` e `messages` — a "baseline de graça" da Fase 0 (`GROUP BY provider, status`) é a `casos_por_dia`/`desfechos` daquele projeto, e `docs/metrics/v3_queries.sql` já tem as consultas de negócio; a lição do `created_at` (hora da finalização ≠ hora do pedido) tem paralelo direto: a mensagem N5 é criada por `resolve_elapsed_autonomous_sends()` numa requisição posterior — `created_at` da mensagem é o envio, não o pedido; a espera é `message.created_at − triggering_message.created_at`, e é isso que `n5.response_latency_ms` mede. E o `SEM_AVALIACAO` tem primo: `AIGeneration` com provider `deterministic-test` ou `unavailable` não é "resposta ruim"; é outra categoria, e um agregado que a esconda mente. **Dentro de cada caso:** o Langfuse, com `environment=production` quando chegar lá — o argumento "Cloud torna trivial instrumentar produção depois" da revisão. E a Camada 0 é idêntica: limite mensal na OpenAI; `n5_kill_switch_enabled` como disjuntor de produto; o free tier do Langfuse com limite de observações — o que acontece acima dele depende da conta. "Custo por caso" ali é soma das generations por `session_id` dividida pelas conversas concluídas no banco — por conversa, não por trace, porque um turno de fallback tem duas gerações e uma conversa tem vários turnos.

**Em projetos comuns do ecossistema.** Três fontes, três denominadores, um teto — em qualquer stack: métricas de infraestrutura para tráfego (inclusive robôs); o banco da aplicação para uso concluído (só os concluídos aparecem — *survivorship*); a plataforma de observabilidade para custo, erro e tempo por execução. Antes de ligar tracing em produção: o que sai (capítulo 19), para qual projeto (não o de estudo), com qual retenção, e quem tem acesso. Ligar é uma flag; decidir ligar não é.

**O fator humano — onde a IA faz e onde você decide.** Um assistente escreve as sete consultas SQL, o script somente-leitura, o `gcloud logging read` — e as duas armadilhas deste capítulo (`created_at` errado, `NULL` escondido) foram encontradas por **alguém desconfiar do resultado** (−0,01 s de espera humana não existe). Esse "desconfiar" é o diferencial: a IA entrega o número; você sabe que ele é impossível. Foque em: (1) para cada número, perguntar "isso é plausível para o que eu sei do sistema?" antes de perguntar "como calculei?"; (2) manter as **contas** — OpenAI, Google Cloud, Langfuse, LangSmith — sob sua responsabilidade direta: teto, alerta, acesso, cartão; nenhum assistente deve ter, nem precisar ter, poder sobre isso; (3) decidir **quando** avançar: "E4 só faz sentido quando a camada 2 mostrar casos suficientes" é uma decisão de proporção — instrumentar avaliação online para um app que ninguém usa é trabalho de ferramenta, não de produto. Na plataforma, a mesma decisão está em "RAGFlow/Elasticsearch segurados até a Fase 0". E, por fim, o que este capítulo pede toda semana — abrir três fontes e anotar quatro números — é uma rotina que a IA pode preparar e que só vale se uma pessoa olhar. Um painel que ninguém lê é igual a nenhum painel; a leitura é a parte humana, e é a que transforma "temos observabilidade" em "sabemos o que está acontecendo".
