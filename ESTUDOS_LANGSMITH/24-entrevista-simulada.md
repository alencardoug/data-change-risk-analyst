...
Explicar o que se consegue demonstrar — com conceito, evidência, trade-off e limite — não é uma habilidade de entrevista; é a habilidade de **defender uma decisão técnica** em qualquer projeto, diante de qualquer pessoa.

**Na plataforma de atendimento (Langfuse Cloud).** As seis perguntas essenciais têm respostas daquele projeto, e vale treiná-las em paralelo, porque são as que um colega, um gestor ou um entrevistador faria sobre ele. *Como as ferramentas entram?* — FastAPI + pgvector + OpenAI direto, sem LangChain/LangGraph por decisão registrada (segurados até a Fase 0); Langfuse Cloud como interface de diagnóstico, Postgres como fonte de verdade. *O que um trace prova?* — que o `rag.retrieve` trouxe X no rank Y e o modelo recebeu Z; não que a resposta ajudou o paciente — para isso, rubrica. *Quem mantém a conversa viva?* — o Postgres e a janela de veto; o Langfuse observa `pending_opened`/`pending_resolved`, não retoma nada. *Como detecta regressão?* — ainda não detecta: o contrato do runner e do gate está escrito, não implementado — e dizer isso é a resposta certa. *Custo?* — preços de `gpt-5-mini`/`text-embedding-3-small` cadastrados com fonte datada; duas gerações por turno de fallback; custo por conversa aceitável, não por generation; disjuntor na OpenAI. *Juiz?* — 10/12 de concordância planejada, por critério, com inconclusivos separados; **nenhum juiz calibrado ainda**. A demo de três minutos dali: uma conversa N5 ruim, o trace com `autonomy.decision`, o `rag.retrieve` mostrando o documento certo no rank 5, e a ficha de caso — "eu medi, encontrei este problema, e a próxima mudança é esta".

**Em projetos comuns do ecossistema.** A rubrica de quatro pontos (conceito, evidência, trade-off, limite) serve para qualquer relatório técnico, PR description ou ADR. "Se disse 'validei em produção' mostrando fixtures, corrija" é a regra de integridade que mais vale a pena internalizar — os projetos que quebram costumam quebrar por uma afirmação dessas que ninguém conferiu.

**O fator humano — onde a IA faz e onde você decide.** Este capítulo é o mais fácil de terceirizar e o mais inútil terceirizado. Um assistente escreve as seis respostas em tom perfeito — e é exatamente o problema: a resposta que "só funciona com o arquivo aberto" e a resposta gerada têm o mesmo defeito, **não são suas**. O que o entrevistador (ou o colega) testa com "mostra?" é se você sabe o que abrir; a IA não estará lá para abrir. Foque em: (1) gravar-se respondendo sem texto, e marcar os quatro pontos você mesmo — o exercício do capítulo, sem atalho; (2) usar a IA como **entrevistador adversarial**, não como autor: peça que faça a pergunta seguinte, a que desmonta a resposta, "e se o juiz estiver errado?", "e se o slice tiver dois casos?" — é um uso legítimo e poderoso; (3) manter a linha entre o que executou e o que leu: as respostas modelo dizem "neste estudo executei", "sem `--real`, você examinou a rubrica mas ainda não mediu" — essa precisão é o diferencial que a fluência gerada não tem. Na plataforma, a frase equivalente é "a instrumentação está na branch, a Fase 0 ainda não leu 20 conversas, o juiz ainda não foi calibrado" — dita sem constrangimento, porque é verdade e porque mostra que você sabe o que falta.
